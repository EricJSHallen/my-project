...
while IFS=$'\t' read -r case_id run_name st1_file st2_file case_dir; do
  {
    ...
    rm -rf "$case_dir/netlist" "$case_dir/psf" "$case_dir/output_signals.txt.tmp"
...
def spectre_string(path: str) -> str:
    # Spectre parameter values used by vsource file=... must be strings.
    # The template keeps bare placeholders, so insert quoted absolute paths.
    return '"' + path.replace('\\', '\\\\').replace('\"', '\\"') + '"'

st1 = spectre_string(sys.argv[2])
st2 = spectre_string(sys.argv[3])
for p in root.rglob('*'):
    if not p.is_file():
        continue
    try:
...
PY
    ...
    if "$RUN_DIR/run_export_case.sh" "$case_dir"; then
      ...
      echo "FAILED case_id=$case_id export_rc=$?"
      echo "case_id=$case_id" >> "$RUN_DIR/worker_state/job_${JOB_INDEX}_failed.txt"
    fi
  } >> "$LOG" 2>&1
done < "$ASSIGNED_TSV"
