...
# Insert a 1-ohm current-sense resistor between I56's output pin and ground.
# This avoids OCEAN branch-current-name ambiguity. Because R=1 ohm,
# v(i56_iout) is numerically equal to I56 output current in amperes.
# This changes the old hard ground into a 1-ohm shunt; for expected small
# synapse currents the perturbation should be negligible. Use a smaller
# RSENSE_I56 value and scale the exporter if you later need an even stiffer
# virtual ground.
ns, n_sense = re.subn(
...
if n_sense != 1 and 'RSENSE_I56' not in ns:
    raise SystemExit('ERROR: could not insert RSENSE_I56 current-sense resistor before I56')

# Keep public-node saving. We now export current by reading v(i56_iout),
# so no branch-current saving or ambiguous OCEAN current name is required.
if re.search(r'^saveOptions\s+options\b.*$', ns, flags=re.M):
    ns = re.sub(r'^saveOptions\s+options\b.*$', 'saveOptions options save=allpub', ns, count=1, flags=re.M)
else:
    ns += '\nsaveOptions options save=allpub\n'

# Save voltages and the current-proportional sense node explicitly.
ns = re.sub(r'^save\s+.*$', 'save vpre vpre1 i56_iout', ns, count=1, flags=re.M) if re.search(r'^save\s+', ns, flags=re.M) else ns + '\nsave vpre vpre1 i56_iout\n'
...
if 'RSENSE_I56' not in ns: raise SystemExit('ERROR: RSENSE_I56 was not inserted into input.scs')
if re.search(r'^save\s+.*i56_iout', ns, flags=re.M) is None: raise SystemExit('ERROR: i56_iout is not in the save statement')
...
print('verified current sense resistor: RSENSE_I56 (i56_iout 0) resistor r=1')
print('verified current proxy signal: save ... i56_iout')
PY
echo "Imported template into $TEMPLATE"
echo "Template placeholders, parameters, and I56 current-proxy saving verified."
